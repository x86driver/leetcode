...
HashMap* HashMapCreate() {
    HashMap *hash = (HashMap*)calloc(1, sizeof(*hash));
    return hash;
}
...
HashNode *getHashNode(HashMap* obj, int key) {
    int pos = Hash(key);
    return obj->node[pos];
}

void setHashNode(HashMap* obj, int key, HashNode *node) {
    int pos = Hash(key);
    assert(obj->node[pos] == NULL);
    obj->node[pos] = node;
}

HashKV *searchKV(HashMap* obj, int key) {
